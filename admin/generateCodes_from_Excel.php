...
function load_data_from_excel($conn, $filename, $email_sender, $email_password) {
    ...
    $valid_rows = 0; // Counter for valid rows

    for ($row = 2; $row <= $highest_row; $row++) {
        $mobile = $worksheet->getCellByColumnAndRow(1, $row)->getValue();
        $email = $worksheet->getCellByColumnAndRow(2, $row)->getValue();

        // Skip row if mobile or email is empty
        if (empty($mobile) || empty($email)) {
            continue;
        }
        
        $valid_rows++; // Increment valid row counter
        
        // Check if mobile starts with 0, if not, prepend 0
        ...
        $stmt = $conn->prepare(
            "INSERT INTO regcode (mobile, email, user_level, code)
            VALUES (?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE email = VALUES(email), user_level = VALUES(user_level), code = VALUES(code)"
        );
    ...
    echo $valid_rows; // Echo the number of valid non-empty rows
}
